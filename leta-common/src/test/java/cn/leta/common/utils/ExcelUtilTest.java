...
import cn.leta.common.poi.reader.IRowReader;
...
public class ExcelUtilTest {
    ...
    private Map<String, List<User>> getDataMap() throws ParseException {
        ...
        sheet1.add(new User("张三", 12, DateUtils.parseDate("1990-11-02", DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern())));
        sheet1.add(new User("李四", 11, DateUtils.parseDate("1990-09-02", DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern())));
        sheet1.add(new User("王五", 13, DateUtils.parseDate("1990-12-02", DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern())));
        sheetDateMap.put("一班", sheet1);
        List<User> sheet2 = new ArrayList<>();
        sheet2.add(new User("Lily", 12, DateUtils.parseDate("1990-06-30", DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern())));
        sheet2.add(new User("Andy", 11, DateUtils.parseDate("1990-09-12", DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern())));
        sheet2.add(new User("Jack", 13, DateUtils.parseDate("1990-12-12", DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern())));
    ...
    public void writeExcelByTemplateTest() throws Exception {
        ...
        ExcelUtil.writeExcelByTemplate(tmp, "/tmp/tmpdata.xlsx", (row, user) -> {
            ...
            row.createCell(cellIndex++).setCellValue(user.getName());
            row.createCell(cellIndex++).setCellValue(user.getAge());
            row.createCell(cellIndex++).setCellValue(DateFormatUtils.format(user.getBirthDay(), DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern()));
        }, getDataMap());
    }
